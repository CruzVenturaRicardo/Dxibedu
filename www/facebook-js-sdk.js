...
FB.provide('ApiServer', {
   ...
   *   FB.api('/me', function(r) { alert(r) })
   *   FB.api('/me', { fields: 'email' }); // throw away response
   *   FB.api('/me', { fields: 'email' }, function(r) { alert(r) });
   *   FB.api('/12345678', 'delete', function(r) { alert(r) });
   *   FB.api(
   *     '/me/feed',
   *     'post',
   *     { body: 'hi there' },
...
FB.provide('Canvas', {
   ...
   *     alert('I will process some achievement now.');
   *   } else {
  ...
  prepareParams: function(call) {
    ...
    if (FB.TemplateUI && FB.TemplateUI.supportsTemplate(method, call)) {
      ...
      if (FB.reportTemplates) {
        alert("Using template for " + method + ".");
      }
